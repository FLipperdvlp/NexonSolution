...
def parse_rep_message(text: str) -> Optional[Tuple[str, str, str]]:
    ...
    Возвращает (знак, идентификатор_цели, описание) или None.
    Идентификатор — это @username ИЛИ числовой telegram_id, как ввёл автор.
    """
...
def display_name(target_id: int, username: str = "") -> str:
    """Как показывать цель пользователю: @username, если известен, иначе ID."""
...
async def resolve_target(identifier: str) -> Tuple[Optional[int], str]:
    """
    Принимает то, что ввёл пользователь (с @ или без, число или username),
    возвращает (target_id, username_для_отображения).
    target_id будет None, если распознать пользователя не удалось.
    """
...
async def help_handler(message: types.Message) -> None:
    help_text = (
        ...
        "• Команда: <code>/check @username</code> или <code>/check 123456789</code>\n"
...
async def reputation_handler(message: types.Message) -> None:
    ...
    # --- Резолвим цель отзыва (username или id) в реальный target_id ---
...
async def channel_reputation_handler(message: types.Message) -> None:
    ...
    logger.info(
        f"Канальный отзыв #{review_id}: {sign} реп target_id={target_id} "
        f"из канала '{chat.title}' (id={chat.id})"
    )


# ---------------------------------------------------------------------------
# CALLBACK: кнопка «Проверить продавца» → запрос username/id через FSM
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# FSM: обработка введённого username/id после нажатия кнопки
# ---------------------------------------------------------------------------
...
async def top_callback(callback: CallbackQuery) -> None:
    ...
    for i, user in enumerate(top_users):
        ...
        label = display_name(user["target_id"], user["username"])
        lines.append(
            f"{medal} <b>{label}</b>\n"
            f"   {bar}\n"
...
async def admin_handler(message: types.Message) -> None:
    ...
    admin_text = (
        ...
        "   — Удалить отзыв по ID\n\n"
        "🚫 <code>/banuser @username|id причина</code>\n"
...
# ---------------------------------------------------------------------------
# ADMIN: /banuser — бан пользователя (по @username или ID)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# ADMIN: /unban — разбан пользователя (по @username или ID)
# ---------------------------------------------------------------------------
